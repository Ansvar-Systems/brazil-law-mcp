#!/usr/bin/env tsx
/**
 * Database builder for Brazil Law MCP server.
 *
...
type EUReferenceType = 'implements' | 'modeled_on' | 'references';
...
-- FTS5 for provision search (Portuguese unicode support)
...
CREATE TABLE cross_references (
  ...
  ref_type TEXT NOT NULL DEFAULT 'references'
    CHECK(ref_type IN ('references', 'amended_by', 'implements', 'see_also', 'modeled_on'))
);
...
-- EU/International reference documents
...
-- EU References (links Brazilian provisions to EU/international documents)
CREATE TABLE eu_references (
  ...
  reference_type TEXT NOT NULL CHECK (reference_type IN (
    'implements', 'supplements', 'applies', 'references', 'modeled_on',
...
function extractArticleReference(context: string): string | null {
  const match = context.match(/\bArt(?:icle|igo)?\s+(\d+[A-Za-z]?(?:\(\d+\))?)/i);
  return match ? match[1] : null;
}

function inferReferenceType(context: string): EUReferenceType {
  if (/\b(model(?:ed|ado)|inspir(?:ed|ado)|bas(?:ed|eado))\b/i.test(context)) {
    return 'modeled_on';
  }
  if (/\b(implement|transpos|supplement|complies?|gives effect)\b/i.test(context)) {
    return 'implements';
  }
  return 'references';
}
...
function extractEuReferences(text: string): ExtractedEUReference[] {
  ...
  const patterns: RegExp[] = [
    /\b(Regulation|Directive|Regulamento|Diretiva)\s*\((EU|EC|EEC|Euratom|UE)\)\s*(?:No?\.?\s*)?(\d{2,4})\/(\d{1,4})\b/gi,
    /\b(Regulation|Directive|Regulamento|Diretiva)\s*(?:No?\.?\s*)?(\d{2,4})\/(\d{1,4})\/(EU|EC|EEC|Euratom|UE)\b/gi,
    /\b(Regulation|Directive|Regulamento|Diretiva)\s*(?:No?\.?\s*)?(\d{2,4})\/(\d{1,4})\b/gi,
    /\bGDPR\b/gi,
  ];
  ...
  for (const pattern of patterns) {
    ...
    while ((match = pattern.exec(text)) !== null) {
      // Special handling for GDPR keyword
      if (match[0].toUpperCase() === 'GDPR') {
        const dedupeKey = 'regulation:2016/679:';
        ...
        refs.push({
          ...
          year: 2016,
          number: 679,
          euDocumentId: 'regulation:2016/679',
          euArticle: null,
          fullCitation: 'GDPR (Regulation (EU) 2016/679)',
          referenceContext,
          referenceType: 'modeled_on',
        });
        continue;
      }

      const typeRaw = match[1].toLowerCase();
      const type: EUDocumentType = (typeRaw === 'regulation' || typeRaw === 'regulamento') ? 'regulation' : 'directive';
      ...
      // Normalize UE to EU
      if (communityRaw?.toUpperCase() === 'UE') communityRaw = 'EU';
...
function buildDatabase(): void {
  console.log('Building Brazil Law MCP database...\n');
        ...
        if (extractedRefs.length > 0) {
          ...
          for (const ref of extractedRefs) {
            ...
            const euInsert = insertEuDocument.run(
              ...
              shortName, shortName, eurLexUrl,
              'Auto-extracted from Brazilian law text',
            );
            if (euInsert.changes > 0) totalEuDocuments++;

            const primaryKey = `${seed.id}:${ref.euDocumentId}`;
            const isPrimary =
              (ref.referenceType === 'modeled_on' || ref.referenceType === 'implements') && !primaryImplementationByDocument.has(primaryKey) ? 1 : 0;
            if (isPrimary === 1) primaryImplementationByDocument.add(primaryKey);

            try {
              const refInsert = insertEuReference.run(
                'provision', sourceId, seed.id, provisionId,
                ref.euDocumentId, ref.euArticle, ref.referenceType,
                ref.referenceContext, ref.fullCitation, isPrimary,
  ...
  const writeMeta = db.transaction(() => {
    ...
    insertMeta.run('jurisdiction', 'BR');
    insertMeta.run('source', 'planalto.gov.br');
    insertMeta.run('licence', 'Brazilian Government Public Domain');
  });
  writeMeta();
