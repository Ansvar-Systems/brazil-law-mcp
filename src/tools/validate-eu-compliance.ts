/**
 * validate_eu_compliance — Check a Brazilian law's EU/international compliance status.
 *
 * Useful for assessing how closely Brazilian law aligns with EU frameworks
 * (e.g., LGPD alignment with GDPR).
 */
...
export async function validateEUCompliance(
  ...
  if (input.provision_ref?.trim()) {
    const normalizedRef = input.provision_ref
      .replace(/^art\.?\s*/i, '')
      .replace(/[ºªo°]+$/i, '')
      .trim();
    const artRef = `art${normalizedRef}`;

    const row = db.prepare(
      'SELECT id FROM legal_provisions WHERE document_id = ? AND (provision_ref = ? OR provision_ref = ? OR section = ?) LIMIT 1'
    ).get(resolvedId, input.provision_ref, artRef, normalizedRef) as { id: number } | undefined;
  ...
  if (rows.length === 0) {
    recommendations.push(
      'No EU/international references found. If this law is modeled on EU legislation, consider adding cross-references.'
    );
  } else if (primaryCount === 0) {
    warnings.push('EU/international references exist, but none are marked as primary basis.');
    recommendations.push('Review reference quality and mark the primary EU basis (e.g., LGPD modeled_on GDPR).');
  }
