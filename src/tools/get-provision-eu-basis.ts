/**
 * get_provision_eu_basis — Get EU/international legal basis for a specific Brazilian provision.
 */
...
export async function getProvisionEUBasis(
  ...
  const provision = db.prepare(
    'SELECT id, content FROM legal_provisions WHERE document_id = ? AND (provision_ref = ? OR provision_ref = ? OR section = ?)'
  ).get(resolvedId, input.provision_ref, artRef, normalizedRef) as { id: number; content: string } | undefined;
