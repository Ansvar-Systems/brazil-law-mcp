/**
 * search_eu_implementations — Search EU directives/regulations with Brazilian implementation info.
 */
...
export interface SearchEUImplementationsInput {
  ...
  year_from?: number;
  year_to?: number;
  has_brazilian_implementation?: boolean;
  limit?: number;
}
...
export async function searchEUImplementations(
  ...
  let sql = `
    SELECT
      ...
      COUNT(DISTINCT er.document_id) AS brazilian_law_count,
  ...
  if (input.has_brazilian_implementation !== undefined) {
    sql += input.has_brazilian_implementation ? ` HAVING brazilian_law_count > 0` : ` HAVING brazilian_law_count = 0`;
  }
  ...
  interface Row {
    ...
    brazilian_law_count: number; primary_implementations: string | null;
  }
  ...
  return {
    results: {
      results: rows.map(r => ({
        ...
        brazilian_law_count: r.brazilian_law_count,
